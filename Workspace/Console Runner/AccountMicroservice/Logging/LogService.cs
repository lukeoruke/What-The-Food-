namespace Console_Runner.Logging
{
    public class LogService
    {
        ...
        public async Task<Log> LogAsync(string actorID, LogSeverity level, Category category, DateTime timestamp, string message, int timeout = -1)
        {
        ...
        /// <param name="timeout">The time in milliseconds that are allowed to elapse before the log attempt is considered failed.</param>
        /// <returns>A Log object representing the log entry to be written to the database.</returns>
        public async Task<Log> LogWithSetUserAsync(LogSeverity level, Category category, DateTime timestamp, string message, int timeout = -1)
        {
            if(UserID == null)
            {
