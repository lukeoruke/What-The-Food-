    ...
    public class AccountDBOperations
    {
        ...
        public async Task<bool> UserSignUpAsync(Account acc, LogService? logService = null)
        {
            try
            {
                ...
                {
                    _ = logService.LogWithSetUserAsync(Logging.LogSeverity.Debug, Category.Business, DateTime.Now,
            ...
            catch (Exception ex)
            {
                if (logService?.UserID != null)
                {
                    _ = logService.LogWithSetUserAsync(LogSeverity.Warning, Category.Business, DateTime.Now,
        ...
        public async Task<bool> UserDeleteAsync(Account currentUser, int userID, LogService? logService = null)
        {
            try
            {
                ...
                if (! await _accountAccess.AccountExistsAsync(userID, logService))
                {
                    if (logService?.UserID != null)
                    {
                        _ = logService.LogWithSetUserAsync(LogSeverity.Error, Category.Business, DateTime.Now,
                ...
                if (logService?.UserID != null)
                {
                    _ = logService.LogWithSetUserAsync(LogSeverity.Debug, Category.Business, DateTime.Now,
                                                       $"Deleted user {userID}.");
                }
                return true;
            }
        ...
        public async Task<Account> SignInAsync(string email, string userPass, LogService? logService = null)
        {
            ...
            if (await AuthenticateUserPassAsync(email, userPass))
            {
                ...
                if (logService?.UserID != null)
                {
                    _ = logService.LogWithSetUserAsync(LogSeverity.Info, Category.Business, DateTime.Now,
